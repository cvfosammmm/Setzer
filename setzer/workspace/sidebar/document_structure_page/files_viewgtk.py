...
class FilesSectionView(structure_widget.StructureWidget):
    ...
    def do_snapshot(self, snapshot):
        ...
        self.draw_hover_background(snapshot, len(self.model.includes) + 1)

        snapshot.translate(Graphene.Point().init(9, 13))

        self.icons['file-symbolic'].snapshot_symbolic(snapshot, 16, 16, [self.fg_color])
        self.layout.set_width((self.get_allocated_width() - 47) * Pango.SCALE)
        self.layout.set_text(os.path.basename(self.model.data_provider.document.get_displayname()))
        snapshot.translate(Graphene.Point().init(26, -1))
        snapshot.append_layout(self.layout, self.fg_color)
        snapshot.translate(Graphene.Point().init(-(26), self.line_height + 1))

        snapshot.translate(Graphene.Point().init(18, 0))
        for count, include in enumerate(self.model.includes):
            self.icons['file-symbolic'].snapshot_symbolic(snapshot, 16, 16, [self.fg_color])
            self.layout.set_width((self.get_allocated_width() - 65) * Pango.SCALE)
            self.layout.set_text(os.path.basename(include['filename']))
            snapshot.translate(Graphene.Point().init(26, -1))
            snapshot.append_layout(self.layout, self.fg_color)
    ...
    def setup_icons(self, widget=None):
        ...
        icon = icon_theme.lookup_icon('file-symbolic', None, 16, self.get_scale_factor(), Gtk.TextDirection.LTR, 0)
        self.icons['file-symbolic'] = icon
