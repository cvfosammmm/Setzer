...
class ShortcutsBar(Gtk.HBox):
    ...
    def insert_math_button(self):
        ...
        self.add_separator(box)
        self.add_action_button(box, 'Inline Math Section ($ ... $)', 'win.insert-before-after', ['$ ', ' $'])
        self.add_action_button(box, 'Display Math Section (\\[ ... \\])', 'win.insert-before-after', ['\\[ ', ' \\]'])
        self.add_menu_button(box, 'Math Environments', 'math_environments')
        self.add_separator(box)
        self.add_action_button(box, 'Subscript (_{})', 'win.insert-before-after', ['_{', '}'])
        self.add_action_button(box, 'Superscript (^{})', 'win.insert-before-after', ['^{', '}'])
        self.add_action_button(box, 'Fraction (\\frac)', 'win.insert-symbol', ['\\frac{•}{•}'])
        self.add_action_button(box, 'Square Root (\\sqrt)', 'win.insert-symbol', ['\\sqrt{•}'])
        self.add_action_button(box, '\\left', 'win.insert-symbol', ['\\left •'])
        self.add_action_button(box, '\\right', 'win.insert-symbol', ['\\right •'])
        self.add_separator(box)
